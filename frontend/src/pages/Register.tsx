...
export default function Register() {
  ...
  const activeTaskStats = task ? [
    ...
    { label: 'Progress', value: task.progress || '0/0', icon: Workflow },
    { label: 'Success', value: String(task.success ?? 0), icon: CheckCircle },
    { label: 'Failed', value: String(task.error_count ?? task.errors?.length ?? 0), icon: XCircle },
  ] : []
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_340px]">
        <div className="space-y-4">
          <Card>
            <CardHeader><CardTitle>Basic Config</CardTitle></CardHeader>
          ...
          <Card>
            <CardHeader><CardTitle>Step 1 · Registration Identity</CardTitle></CardHeader>
          ...
          <Card>
            <CardHeader><CardTitle>Step 2 · Execution Channel</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              ...
              {form.identity_provider === 'oauth_browser' && (
                <>
                  <Input label="Expected login email (optional)" k="oauth_email_hint" placeholder="your-account@example.com" />
                  <Input label="Chrome Profile path" k="chrome_user_data_dir" placeholder="~/Library/Application Support/Google/Chrome" />
                  <Input label="Chrome CDP address" k="chrome_cdp_url" placeholder="http://localhost:9222" />
                  <p className="text-xs text-[var(--text-muted)]">
                    When using background browser for third-party accounts, configure Chrome Profile or Chrome CDP to reuse the locally logged-in browser session.
                  </p>
                </>
              )}
            </CardContent>
          ...
          {form.identity_provider === 'mailbox' && (
            <Card>
              <CardHeader><CardTitle>System Email Config</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {optionsError && (
                ...
                {mailboxProviderOptions.length > 0 ? (
                  <Select label="Email Service" k="mail_provider" options={mailboxProviderOptions} />
                ) : (
                  <div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
                    No enabled email provider found. Please add and enable a default email provider in Settings first.
                  </div>
                )}
          ...
          {smsProviderOptions.length > 0 && (
            <Card>
              <CardHeader><CardTitle>SMS Config</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {optionsError && (
                  <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-300">
                    {optionsError}
                ...
                <Select label="SMS Service" k="sms_provider" options={smsProviderOptions} />
        ...
        <div className="space-y-5 xl:sticky xl:top-4 xl:self-start">
          <Card className="bg-[var(--bg-pane)]/62">
            <CardHeader>
              <CardTitle>Current Orchestration Summary</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Button onClick={submit} disabled={polling} className="w-full">
                {polling ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Registering...</> : <><Play className="mr-2 h-4 w-4" />Start Registration</>}
              </Button>
            </CardContent>
          </Card>
          ...
          {task ? (
                ...
                <CardContent className="space-y-4">
                  ...
                  {task.status === 'interrupted' && !task.error && (
                    <div className="flex items-center gap-2 text-amber-400">
                      <XCircle className="h-4 w-4" />
                      <span className="text-xs">Task interrupted after service restart</span>
                    </div>
                  )}
            ...
            <Card className="bg-[var(--bg-pane)]/55">
              <CardHeader><CardTitle>Waiting for Execution</CardTitle></CardHeader>
              <CardContent className="text-sm text-[var(--text-secondary)]">Status and logs will appear after creation.</CardContent>
            </Card>
          )}
        </div>
      </div>
